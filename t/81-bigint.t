...
subtest 'basic mod ops', sub {
  ...
  my $Ca = 10540222481731305179;   # a composite
  ...
  my @all3 = qw/1653087340990329197 15231025166435087123 28808962991879845049 42386900817324602975 55964838642769360901 69542776468214118827 83120714293658876753/;
  ok(is_one_of(rootmod($D,35,$Da),@all3),"rootmod(C,35,Da) [composite modulus]");

  is_deeply([map{"$_"}allsqrtmod($C,$D)],[8765086989354512719,86280477788758792438],"allsqrtmod");
  is_deeply([map{"$_"}allrootmod($D,35,$Da)], \@all3, "allrootmod");
};
...
subtest 'gcdext and chinese', sub {
  ...
  is_deeply( [map{"$_"}gcdext(803028077895224634710139483024654235947,101394830246542359478030280778952246347)], [7687627417944666569835322284775478836, -60884570288210047004733169112173096587, "3"], "gcdext(a,b)" );
...
subtest 'range primes', sub {
  ...
  is_deeply( [map{"$_"}@{primes(73786976294838206464, 73786976294838206564)}],
             [73786976294838206473,73786976294838206549],
             "primes( 2^66, 2^66 + 100 )" );

  is_deeply( [map{"$_"}@{twin_primes(18446744073760736000,18446744073760737000)}],
             [18446744073760736057,18446744073760736717],
             "twin_primes(18446744073760736000,+1000)" );
};
...
subtest 'factoring', sub {
  ...
  ($n,@f) = (1234567890, 2,3,3,5,3607,3803);
  is_deeply([factor($n)],\@f,"factor($n)");
  is_deeply([factor_exp($n)],[linear_to_exp(@f)],"factor_exp($n)");

  ($n,@f) = (3493005066479, 61,101,1063,533353);
  is_deeply([factor($n)],\@f,"factor($n)");
  is_deeply([factor_exp($n)],[linear_to_exp(@f)],"factor_exp($n)");
...
subtest 'liouville', sub {
  is( liouville(14964156817349780894766767),"-1", "liouville(a x b x c) = -1" );
  is( liouville(   14277153908541447331733), "1", "liouville(a x b x c x d) = 1" );
};
###############################################################################
